...
import { X, Save, Shield, Key, Cpu, Eye, EyeOff, Download, Cloud, HardDrive, CheckCircle2, AlertTriangle, ExternalLink } from 'lucide-react';
...
const SettingsDialog: React.FC<SettingsDialogProps> = ({ isOpen, onClose, settings, onSave }) => {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'drive'>('general');
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex bg-zinc-800/50 rounded-lg p-1 border border-zinc-700/50">
                <button 
                    ...
                    className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${activeTab === 'general' ? 'bg-zinc-700 text-white shadow-sm' : 'text-zinc-400 hover:text-zinc-200'}`}
                >
                    General
                </button>
                <button 
                    onClick={() => setActiveTab('drive')}
                    className={`px-3 py-1 text-xs font-medium rounded-md transition-all flex items-center gap-1 ${activeTab === 'drive' ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-400 hover:text-zinc-200'}`}
                >
              ...
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                  <div className="bg-amber-900/20 border border-amber-900/50 p-4 rounded-xl flex gap-3">
                      ...
                      <div className="text-sm text-amber-200/80">
                          <p className="font-semibold text-amber-100 mb-1">Configuration Required</p>
                          To access your Google Drive folders securely, you must create a Project in Google Cloud and provide your own credentials.
                      </div>
                  </div>
                  ...
                  <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-medium text-zinc-400 flex items-center gap-1">
                                <HardDrive size={12} /> OAuth 2.0 Client ID
                            </label>
                            <div className="relative">
                                <input
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-medium text-zinc-400 flex items-center gap-1">
                                <Key size={12} /> Google Cloud API Key
                            </label>
                            <div className="relative">
                                <input
                            ...
                            <p className="text-[10px] text-zinc-500">Required for the Google Picker API to function.</p>
                        </div>
                  </div>

                  {/* Setup Checklist */}
                  <div className="mt-6 border-t border-zinc-800 pt-6">
                      <h3 className="text-sm font-bold text-white mb-4">Setup Checklist</h3>
                      <ul className="space-y-3">
                          {[
                              "Go to console.cloud.google.com and create a project.",
                              "Go to 'Library' and ENABLE 'Google Drive API'.",
                              "Go to 'Library' and ENABLE 'Google Picker API'. (Critical)",
                              "Go to 'Credentials' → Create 'API Key'. Paste it above.",
                              "Go to 'Credentials' → Create 'OAuth Client ID' (Web App).",
                              "Add this URL to 'Authorized JavaScript origins' and 'Redirect URIs'.",
                              "Paste the Client ID above."
                          ].map((step, i) => (
                              <li key={i} className="flex items-start gap-3 text-xs text-zinc-400">
                                  <div className="mt-0.5 min-w-[16px] h-4 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-[10px] font-mono">
                                      {i + 1}
                                  </div>
