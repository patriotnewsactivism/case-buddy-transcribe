    ...
    <div className="space-y-2">
      ...
      {progressInfo && (
        <div className="flex items-center justify-between text-[10px] text-zinc-600">
          <span>{progressInfo.message || stage}</span>
          {progressInfo.bytesProcessed && progressInfo.bytesTotal && (
            <span>
              {(progressInfo.bytesProcessed / 1024 / 1024).toFixed(1)}MB / 
              {(progressInfo.bytesTotal / 1024 / 1024).toFixed(1)}MB
            </span>
          )}
        </div>
      )}
      ...
      <div className="flex gap-1 mt-1">
        {(['FETCHING_MEDIA', 'EXTRACTING_AUDIO', 'UPLOADING', 'PROCESSING_AI', 'FINALIZING'] as ProcessingStage[]).map((s) => {
          const isActive = currentStage === s;
          const stageOrder = ['QUEUED', 'FETCHING_MEDIA', 'EXTRACTING_AUDIO', 'UPLOADING', 'PROCESSING_AI', 'FINALIZING', 'COMPLETED', 'ERROR'];
          const isComplete = stageOrder.indexOf(currentStage) > stageOrder.indexOf(s);
          
          return (
            <div 
              key={s}
              className={'flex-1 h-0.5 rounded-full transition-all ' + (isComplete ? 'bg-emerald-500' : isActive ? STAGE_CONFIG[s].color.replace('text-', 'bg-') : 'bg-zinc-800')}
            />
          );
        })}
      </div>
...
`;
fs.writeFileSync('components/ProgressIndicator.tsx', content, 'utf8');
